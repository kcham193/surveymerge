...
Small typos or grammatical errors in documentation can be edited directly
through the GitHub web interface, provided the changes are made in the
**source** file:

* Fix a typo in roxygen comments inside `R/`, not the generated `.Rd` files
  in `man/`.
...
2. Install all development dependencies with
   ...
   R CMD check by running `devtools::check()`. If it does not pass, ask
...
* New code should follow the tidyverse [style guide](https://style.tidyverse.org).
  ...
  package to apply these styles automatically, but please don't restyle
...
All PRs must pass `devtools::check()` with **0 errors, 0 warnings, and 0 notes**.
